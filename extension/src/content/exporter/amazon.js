(() => {
    ...
    const getPagePrice = () => {
        ...
        return info && Number.isFinite(Number(info.price))
            ? withDeliveryCost({ price: Number(info.price), currency: info.currency || '$', text: root?.textContent || '' })
            : withDeliveryCost({ price: 1, currency: getAmazonCurrency(), text: 'Amazon price fallback' });
    };
