...
- `extension/src/content/*.js` are esbuild entrypoints for marketplace content scripts.
...
- `scripts/build-local.mjs` rebuilds committed local bundles.
- `scripts/build.mjs` creates a minified release folder outside the repo at `../owb-tools-release` unless `RELEASE_DIR` is set.
...
`npm install` runs `prepare`, which sets:
